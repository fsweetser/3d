...
difference(){
    ...
    translate([notchxmargin, notchymargin, 0]){
        mirror([1,0,0]) notch();
    }

    translate([outerx - notchxmargin, notchymargin, 0]){
        notch();
    }
    ...
    translate([outerx/2,15,bottomz]){
        #write("303H", h=10, center=true);
    }
}

translate([postxmargin,lpostboty,outerz]){
    post();
}

translate([outerx-postxmargin,rpostboty,outerz]){
    post();
}
